...
use App\Models\SupportAttendanceModel;

class ConSupportAttendance extends BaseController
{
    protected $attendanceModel;

    public function __construct()
    {
        $this->attendanceModel = new SupportAttendanceModel();
    }

    private function getSupportPersId()
    {
        $session = session();
    ...
    /**
     * แดชบอร์ดต้อนรับสำหรับฝ่ายสนับสนุน
     */
    public function dashboard()
    {
        ...
        // สถานะวันนี้
        $todayStatus = 'ยังไม่ได้ลงเวลา';
        if ($todayRecord) {
            if ($todayRecord['check_out']) {
                $todayStatus = 'ลงเวลากลับสำเร็จ';
            } elseif ($todayRecord['check_in']) {
                $todayStatus = 'ลงเวลาเข้าสำเร็จ (' . $todayRecord['status'] . ')';
            }
        }

        // ดึงสรุปผลของเดือนนี้
        $currentMonth = date('m');
        $currentYear = date('Y');
        $summary = $this->attendanceModel->getMonthlySummary($persId, $currentMonth, $currentYear);
        
        // ดึงรายการประวัติย้อนหลังล่าสุด 5 วัน
        $recentHistory = $this->attendanceModel->where('pers_id', $persId)
                                              ->orderBy('att_date', 'DESC')
                                              ->limit(5)
                                              ->findAll();

        $data = [
            'title'             => 'แดชบอร์ดฝ่ายสนับสนุน',
            'description'       => 'ระบบลงเวลางานและแผงควบคุมฝ่ายสนับสนุน',
        ...
        return view('Support/SupportAttendance/PageSupportDashboard', array_merge($this->data, $data));
    }

    /**
     * หน้าเช็คชื่อหลักสำหรับฝ่ายสนับสนุน
     */
    public function index()
    {
        ...
        $data = [
            'title'             => 'เช็คชื่อเข้างานฝ่ายสนับสนุน',
            'description'       => 'ระบบลงเวลางานออนไลน์สำหรับบุคลากรสายสนับสนุน',
            'todayRecord'       => $todayRecord,
            'status'            => $status,
            'locationSettings'  => $locationSettings,
            'isSystemActive'    => $locationSettings ? (bool)$locationSettings->is_active : true,
        ...
        return view('Support/SupportAttendance/PageSupportAttendance', array_merge($this->data, $data));
    }
